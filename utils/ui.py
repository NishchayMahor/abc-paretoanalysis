...
def introduction():
    st.title('ABC Analysis & Product Segmentation')
...
def upload_ui():
    ...
    if input is None:
        dataset_type = 'LOCAL'
        st.sidebar.write("Just in case you don't upload a dataset on your own, an example is automatically loaded to show you the features of this app :D ")
...
def dataset_ui(df_abc, df, dataset_type):
    ...
    with st.expander("FEATURES FOR THE DATES AND THE VALUES"):
        ...
        with col1:
            date_col = st.selectbox("Select date column",index= len(columns) - 2,options=columns,key="date1")
        with col2:
            metric_col = st.selectbox("Select values column",index=len(columns)-1,options=columns,key="values1")
        output = 0 
    ...
    ''')
        sku_col = st.selectbox("Select values column",index=1,options=list_var,key="values2")
    ...
    ''')
        family_col = st.selectbox("Select a column for the product family", index= 3,options=list_var,key="date2")
...
def export_ui(df_abc):
    ...
    if st.checkbox('Export Data',key='show1'):
